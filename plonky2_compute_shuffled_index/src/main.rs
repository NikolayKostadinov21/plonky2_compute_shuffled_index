...
fn main() -> Result<(), anyhow::Error> {
    ...
    for current_round in 0..SHUFFLE_ROUND_COUNT {
        ...
        let seed_curr_round_target =
            make_circuits(&mut builder, to_be_hashed_seed_round_len as u64);

        let mut hash_of_seed_curr_round = builder.add_virtual_target();
        for i in 0..64 {
            hash_of_seed_curr_round = builder.add(
                hash_of_seed_curr_round,
                seed_curr_round_target.digest[i].target,
            );
        }
        ...
        let source_to_be_hashed_target =
            make_circuits(&mut builder, source_to_be_hashed_len as u64);
        ...
        let exponent_bits = builder.split_le(pos_mod_8, 32);

        let _2_exp_y = builder.exp_from_bits_const_base(F::from_canonical_u8(2), exponent_bits);
        ...
        let i_eq_f = builder.is_equal(index, flip);
        index = builder.select(i_eq_f, bit, index);
    }
